...
# Unauthenticated GitHub is 60 req/hour. The upstream client sleeps until
# reset (up to an hour). That freezes the evaluator. We never wait — we stop
# calling and score with whatever we already have.
_GITHUB_QUOTA_FLOOR = 8
_github_quota_exhausted = False


def reset_github_quota_guard() -> None:
    global _github_quota_exhausted
    _github_quota_exhausted = False
...
def _fetch_github_api(api_url, params=None):
    ...
    if rate_limit_remaining is not None:
        ...
        if remaining <= _GITHUB_QUOTA_FLOOR or status_code in (403, 429):
            _github_quota_exhausted = True
            logger.warning(
                "GitHub quota low (%s remaining, status %s). "
                "Stopping further GitHub calls instead of sleeping until reset. "
                "Set GITHUB_TOKEN to raise the limit (60/hour → 5000/hour).",
                remaining,
                status_code,
            )
...
def fetch_all_github_repos(github_url: str, max_repos: int = 12) -> List[Dict]:
    try:
        username = extract_github_username(github_url)
        if not username:
            print(f"Could not extract username from: {github_url}")
        ...
        if status_code == 200:
            ...
            for repo in repos_data[:max_repos]:
                if _github_quota_exhausted:
                    logger.warning("Stopping repo scan — GitHub quota exhausted")
                    break
                ...
                contributor_count = len(contributors_data) if contributors_data else 1
                ...
                project_type = (
                    "open_source"
                    if contributor_count > 1 or repo.get("fork")
                    else "self_project"
                )
...
def fetch_and_display_github_info(
    ...
    projects_json = generate_projects_json(
        projects, position_title=position_title, model=model, provider=provider
    )
