...
_DEFAULT_TEMPLATES = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "prompts", "templates"
)
...
class PDFHandler:
    def __init__(
        ...
        self.template_manager = TemplateManager(template_dir or _DEFAULT_TEMPLATES)
        self.on_progress = on_progress
        if provider is not None:
            self.provider = provider
            self.model = getattr(provider, "model", None) or DEFAULT_MODEL
        else:
    ...
    def _emit_progress(self, section: str, status: str) -> None:
        if self.on_progress:
            self.on_progress(section, status)
    ...
    def _call_llm_for_section(
        ...
        try:
            ...
            logger.debug(
                f"🔄 Extracting {section_name} section using {self.model}..."
            )

            model_params = MODEL_PARAMETERS.get(
                self.model, {"temperature": 0.1, "top_p": 0.9}
            )
    ...
    def extract_json_from_pdf(self, pdf_path: str) -> Optional[JSONResume]:
        try:
            ...
            if not text_content:
                ...
                self._emit_progress("pdf", "error")
                return None

            self._emit_progress("pdf", "done")
    ...
    def _extract_all_sections_separately(
        ...
        for section_name in sections:
            self._emit_progress(section_name, "running")
            ...
            if section_data:
                complete_resume.update(section_data)
                self._emit_progress(section_name, "done")
                logger.debug(f"✅ Successfully extracted {section_name} section")
                ...
                self._emit_progress(section_name, "error")
                logger.error(
                    "⚠️ Failed to extract basics section. Aborting extraction."
                )
                return None
            else:
                self._emit_progress(section_name, "skipped")
                logger.warning(
                    f"⚠️ Failed to extract {section_name} section; continuing without it."
                )
